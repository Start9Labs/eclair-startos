...
export const vpnConfPath = `${dataDir}/vpn/${vpnIface}.conf`
// Where a Lightning VPN provider's forward lands inside the tunnel.
export const tunnelPort = 9735
...
export function renderWgQuick(c: WireguardConfig, listenPort: number): string {
  ...
  const postUp = [
    ...
    // The provider forwards to 9735; eclair listens on the port it announces.
    ...(listenPort !== tunnelPort
      ? [
          `iptables -t nat -A PREROUTING -i %i -p tcp --dport ${tunnelPort} -j REDIRECT --to-ports ${listenPort}`,
        ]
      : []),
    // Only the peer port is reachable from the tunnel; eclair's API is not.
    ...['iptables', 'ip6tables'].flatMap((t) => [
      `${t} -A INPUT -i %i -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT`,
      `${t} -A INPUT -i %i -p tcp --dport ${listenPort} -j ACCEPT`,
...
for t in iptables ip6tables; do
  ...
  $t -t nat -S PREROUTING 2>/dev/null | grep -- "-i ${vpnIface} " | sed 's/^-A /-D /' | while read -r rule; do $t -t nat $rule; done
done
for fam in -4 -6; do
